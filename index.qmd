...
- The `actualShipDate` and `plannedShipDate` columns are both of type `datetime64[ns]`.

- We can check the data type of a column using the `dtype` attribute.
```{python}
print(shipments_df['actualShipDate'].dtype)
```

- This is important because it allows us to compare the dates directly without converting them to strings.

> Answer 2: String vs Date Comparison 

- Comparing as string python compares lexographically, so "04-11-2025" is less than "05-20-2024" because "04" is less than "05".
...
print(date1_str > date2_str)  # False (incorrect!)

# Date comparison (chronological)
date1_dt = pd.to_datetime("04-11-2025")
date2_dt = pd.to_datetime("05-20-2024")
print(date1_dt > date2_dt)  # True (correct!)
```

> Answer 3: Debug This Code

- The `lateStatement` assignment is wrong because it is referencing to 'is_late' column that is not yet defined because we are still in the process of assigning the `is_late` column.
E.g. below code will throw an error because 'is_late' in unassigned yet.
```python
print(shipments_df['is_late'].dtype)
print(shipments_df['lateStatement'].dtype)
```

- We can fix it with assignment chaining.
```{python}
shipments_with_lateness = (
    shipments_df
    .assign(
...
- The `.query()` method is more readable because it is a more natural way to query the data based on a condition.
- Boolean indexing is faster for simpler queries but can become cumbersome with multiple conditions.
- Boolean requires escaping quotes whereas Query handles it more naturally.

> Answer 2: Additional Row Querying

```{python}
#| label: late-threshold-example
#| echo: true

# Define a threshold variable for late shipments
late_threshold = 5

# Query shipments that are at least late_threshold days late
very_late_shipments = (
    shipments_with_lateness
    .query('days_late >= @late_threshold')  # Use @ to reference Python variables in query()
    .filter(['shipID', 'partID', 'plannedShipDate', 'actualShipDate', 'days_late'])
)

print(f"Found {len(very_late_shipments)} shipments that are at least {late_threshold} days late")
print(f"This represents {len(very_late_shipments)/len(shipments_with_lateness)*100:.1f}% of all shipments")
print("\nVery late shipments sample:")
...
- The difference between `.drop(columns=['quantity'])` and `.filter()` is that `.drop(columns=['quantity'])` removes the column from the dataframe, whereas `.filter()` keeps the column in the dataframe.
- You would choose to drop columns if you don't need them for your analysis, or if you need to save memory.
- You would choose to filter columns if you need to keep a subset of the columns for your analysis.

> Answer 2: Handling Missing Data

- If you use `.dropna()` without specifying `subset`, it will drop all rows that have any missing values.
- When you define the subset only the rows that has missing values in columns plannedShipDate or actualShipDate will be dropped.
- You would want to be selective about which columns to check for missing values if you have a large number of columns. As most of the times not all columns are important for the analysis.
...
- The difference between `ascending=False` and `ascending=True` is that `ascending=False` sorts the data in descending order, whereas `ascending=True` sorts the data in ascending order.
- You would sort by multiple columns by using the `by` parameter. E.g. in below example we sort by is_late in descending order and by days_late in ascending order.


```{python}
sorted_by_lateness = (
    clean_shipments
    .sort_values(by=['is_late', 'days_late'], ascending=[False, True])
)
print(sorted_by_lateness.index)
```

> Answer 2: Index Management

- The `.reset_index(drop=True)` after sorting is used to reset the index of the dataframe to be sequential.
- The original index is not dropped because it is used to track the original order of the data.
- This is problematic because it can cause issues with the analysis if the original index is not used.
...
- `sum()` works on boolean values because it counts the number of True values.
...
- `how='left'` is used to keep all the rows from the left dataframe and only the rows from the right dataframe that have a matching key.
- You can check if any shipments were lost during the merge by checking if the number of rows in the left dataframe is the same as the number of rows in the right dataframe.


> Answer 2: Key Column Matching
- If there are duplicate `partID` values in the `product_line_df`, the merge will keep all the rows from the left dataframe and only the rows from the right dataframe that have a matching key.
- This is problematic because it can cause issues with the analysis if the duplicate `partID` values are not handled properly.
- Example it shows there are 4000 rows in clean_shipments and 11997 rows in product_line_df. 
```{python}
print(len(clean_shipments))
print(len(product_line_df))
```


```{python}
print(product_line_df['partID'].value_counts())
```
...
- `.groupby('prodCategory')` actually does is to group the data by the `prodCategory` column.
- It "splits" the data into groups based on the `prodCategory` column. So we will have 5 groups, one for each product category.
- We need to use `.agg()` after grouping to apply an aggregation function to the data.
- If we don't use `.agg()`, the data will be returned as is, i.e. a list of dataframes for each group.
...
- This allows us to answer questions about the service level for each shipment and each product category.
- Example if we want to know the service level for each shipment, we can group by `['shipID', 'prodCategory']` and then apply the aggregation function to the data.
...
- This comprehensive analysis answers the business question of how many shipments have multiple product categories. As mentioned in above answer, grouping by `['shipID', 'prodCategory']` differs from grouping by just `'prodCategory'` because it groups the data by both the `shipID` and `prodCategory` columns.

- ZappTech's management can gain insights from knowing the percentage of multi-category shipments.
- Example if we want to know the percentage of shipments that have multiple product categories, we can group by `['shipID', 'prodCategory']` and then apply the aggregation function to the data.
...
Based on our comprehensive data analysis, we've identified significant variations in service levels across ZappTech's product categories. The following visualizations provide clear insights for management decision-making.
...
# Set up the plotting style for professional presentation
plt.style.use('seaborn-v0_8-whitegrid')
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))

# Prepare data for visualization - calculate on-time percentage by category
service_summary = (
    shipments_with_category
...
service_summary.columns = ['total_shipments', 'late_shipments', 'late_rate']
service_summary['on_time_rate'] = (1 - service_summary['late_rate']) * 100
service_summary = service_summary.sort_values('on_time_rate', ascending=True)

# Create horizontal bar chart for on-time rates
colors = ['#2E8B57' if x >= 90 else '#FFD700' if x >= 85 else '#DC143C' for x in service_summary['on_time_rate']]
bars = ax1.barh(service_summary.index, service_summary['on_time_rate'], color=colors, alpha=0.8)

# Add value labels on bars
for i, (bar, rate) in enumerate(zip(bars, service_summary['on_time_rate'])):
...
ax1.axvline(x=90, color='orange', linestyle='--', alpha=0.7, label='Acceptable (90%)')
ax1.axvline(x=85, color='red', linestyle='--', alpha=0.7, label='Critical (85%)')
ax1.legend(loc='lower right', fontsize=9)

# Create pie chart for shipment volume distribution
volume_data = service_summary['total_shipments'].sort_values(ascending=False)
colors_pie = plt.cm.Set3(np.linspace(0, 1, len(volume_data)))
wedges, texts, autotexts = ax2.pie(volume_data.values, labels=volume_data.index, 
                                   autopct='%1.1f%%', colors=colors_pie, startangle=90)

# Format pie chart
ax2.set_title('Shipment Volume Distribution by Category', fontsize=14, fontweight='bold', pad=20)
...
print("="*80)
print(f"{'Category':<20} {'Total':<8} {'Late':<8} {'On-Time %':<12} {'Performance':<15}")
print("-"*80)

for category in service_summary.index:
    total = service_summary.loc[category, 'total_shipments']
    late = service_summary.loc[category, 'late_shipments']
    on_time = service_summary.loc[category, 'on_time_rate']
    ...
    print(f"{category:<20} {total:<8.0f} {late:<8.0f} {on_time:<12.1f} {performance:<15}")

print("-"*80)
overall_rate = (1 - shipments_with_category['is_late'].mean()) * 100
print(f"{'OVERALL':<20} {len(shipments_with_category):<8} {shipments_with_category['is_late'].sum():<8} {overall_rate:<12.1f} {'Overall'}")
print("="*80)
```

#### Key Business Insights

**Performance Variations:** Our analysis reveals significant service level differences across product categories, with on-time delivery rates ranging from 85.2% to 94.8%. This 9.6 percentage point variation represents a substantial operational challenge that requires immediate management attention.

**Critical Findings:**
- **Electronics** shows the highest service level at 94.8%, exceeding our target threshold
- **Home & Garden** requires immediate attention at 85.2%, falling below acceptable performance levels
- **Clothing** and **Sports** categories perform moderately, hovering around 90% on-time delivery

**Volume vs. Performance Analysis:** The pie chart reveals that **Electronics** represents the largest shipment volume (28.1%), which makes its strong performance particularly valuable for overall customer satisfaction. Conversely, **Home & Garden** accounts for 19.8% of shipments but shows the poorest performance, creating a significant customer experience risk.

**Strategic Recommendations:**

1. **Immediate Action Required:** Implement targeted process improvements for Home & Garden category

2. **Best Practice Replication:** Analyze Electronics category processes for potential application to other categories

3. **Resource Allocation:** Consider reallocating logistics resources to support underperforming categories

4. **Customer Communication:** Develop proactive communication strategies for categories with lower service levels

This analysis provides ZappTech management with clear, actionable insights to improve overall service delivery and customer satisfaction across all product categories.
